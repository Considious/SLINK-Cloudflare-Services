...
1. Apply `../../permissions/migrations/0004-war-service.sql` and `0005-permission-catalog.sql` to the existing `slink-permissions` D1 database.
...
- Snapshot pending logs and the log route require `slink.war.log` or `admin.*`.
...
for `slink.level`, `slink.war`, and `slink.war.log`. The allowlist is enforced by
